...
window.onload = function() {
    ...
    xhr.onload = (e) => {
        ...
        if(xhr.status == 200) {
            ...
            txt += "<li><input type='radio' name='poll_option' value='" + polls['options'][x].id + "' required>" + polls['options'][x].content + "</li>";
            }
            txt += "</ul>";
            txt += "<input type='hidden' name='poll' value='" + polls['poll'].id +"'>";
            txt += "<button id='btn_vote' type='submit'>Vote</button>";
            txt += "<a href='src/results.php?pollID=" + polls['poll'].id + "'>Results ➡ </a>";
            txt += "</form>"; 
